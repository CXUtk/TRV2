...
void DirectionalLightCalculator::DrawTriangles(const glm::mat4& worldProjection)
{
	...
	{
		if(segment.Id == 138 || segment.Id == 137)
...
void DirectionalLightCalculator::calculateTrianglesForOneLight(const Light& light)
{
	...
	//if (_edges.size() > 90)
	//{
	//	structure.currentRay.Start = sweepCenter;
	//	structure.currentRay.Dir = _vertices[677].GetWorldPos() - sweepCenter;
	//	bool a = cmp(&_edges[137], &_edges[138]);
	//	bool b = cmp(&_edges[138], &_edges[137]);
...
void DirectionalLightCalculator::addOneSegment(PVertex A, PVertex B, bool horizontal, glm::vec2 sweepCenter)
{
	...
	float jud = cross2(glm::normalize(vA), glm::normalize(vB));

	if (std::abs(jud) < LightCommon::EPS) return;
	...
	float& minnTime, PEdge& minnEdge)
{
	...
	if (!structure.EdgeSet->empty())
	{
		...
		//	}
		//}
		auto p = structure.EdgeSet->begin();
		auto edge = *p;
	...
	SweepStructure& structure, glm::vec2 sweepCenter, int start, int end)
{
	...
	if (structure.borderEdges.size()>=2 &&structure.borderEdges[0]->Id == 3 && structure.borderEdges[1]->Id == 1)
	{
		if (true);
	}
	...
	if (end == sz || oldMinSeg != minnEdge->Id)
	{
		...
		if (_shadowTriangles.size() == 47 - 12 - 1)
		{
			if (true);
		}
...
void DirectionalLightCalculator::eraseEdge(SweepStructure& structure, PEdge edge, glm::vec2 sweepCenter)
{
	if (edge->IsBorder())
	{
		...
		assert(p != structure.borderEdges.end());
